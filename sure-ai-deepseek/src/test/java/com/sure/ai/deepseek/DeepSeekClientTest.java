...
/**
 * {@link DeepSeekClient} 与 {@link DeepSeekUtil} 集成测试：本地 HttpServer mock。
 *
 * <p>官方文档：https://api-docs.deepseek.com/</p>
 *
 * @author sureai
 * @since 0.1.0
 */
public class DeepSeekClientTest {
	...
	private static void resetUtil() throws Exception {
		Field f = DeepSeekUtil.class.getDeclaredField("HOLDER");
	...
	private DeepSeekClient newClient() {
		AiConfig cfg = AiConfig.builder().apiKey("ds-key").baseUrl(this.baseUrl).build();
	...
	public void testChat() {
		handle(200, "{\"id\":\"ds1\",\"model\":\"deepseek-chat\",\"choices\":[{\"index\":0,"
			+ "\"message\":{\"role\":\"assistant\",\"content\":\"ok\"},\"finish_reason\":\"stop\"}],"
			+ "\"usage\":{\"prompt_tokens\":5,\"completion_tokens\":3,\"total_tokens\":8}}");
		DeepSeekClient client = newClient();
		ChatResponse resp = client.chat(ChatRequest.builder().model(DeepSeekModels.DEEPSEEK_CHAT)
			.messages(ChatMessage.user("hi")).build());
		assertEquals("Bearer ds-key", this.lastAuth.get());
		assertTrue(this.lastUri.get().endsWith("/chat/completions"));
		assertTrue(this.lastBody.get().contains("\"model\":\"deepseek-chat\""));
	...
	public void testChatStream() {
		String sse = "data: {\"id\":\"ds\",\"choices\":[{\"delta\":{\"role\":\"assistant\",\"content\":\"A\"},\"index\":0}]}\n\n"
			+ "data: {\"id\":\"ds\",\"choices\":[{\"delta\":{\"content\":\"B\"},\"index\":0}]}\n\n"
			+ "data: {\"id\":\"ds\",\"choices\":[{\"delta\":{},\"finish_reason\":\"stop\",\"index\":0}]}\n\n"
			+ "data: [DONE]\n\n";
		handle(ex -> {
		...
		client.chatStream(ChatRequest.builder().model(DeepSeekModels.DEEPSEEK_REASONER)
			.messages(ChatMessage.user("hi")).build(),
			chunk -> {
	...
	public void testAuth401() {
		...
		assertThrows(AiAuthException.class, () -> client.chat(
			ChatRequest.builder().model("deepseek-chat").messages(ChatMessage.user("hi")).build()));
		client.close();
	}
	...
	public void testDefaultBaseUrlWhenNull() {
		DeepSeekClient client = new DeepSeekClient(AiConfig.of("k"));
	...
	public void testUtilConvenience() {
		...
		DeepSeekUtil.init(AiConfig.builder().apiKey("util-key").baseUrl(this.baseUrl).build());
		assertEquals("yo", DeepSeekUtil.chat("deepseek-chat", "hi").firstText());
	...
	public void testUtilMissingEnvKey() {
		assertThrows(AiException.class, DeepSeekUtil::client);
	}

	/** buildConfig 分支。 */
	@Test
	public void testBuildConfig() {
		assertEquals("http://mock", DeepSeekUtil.buildConfig("k", "http://mock").baseUrl());
		assertEquals(null, DeepSeekUtil.buildConfig("k", " ").baseUrl());
		assertThrows(AiException.class, DeepSeekUtil::buildConfigFromEnv);
	}

	/** Models 私有构造器。 */
	@Test
	public void testModelsConstructor() throws Exception {
		java.lang.reflect.Constructor<DeepSeekModels> c = DeepSeekModels.class.getDeclaredConstructor();
		...
		assertEquals("deepseek-reasoner", DeepSeekModels.DEEPSEEK_REASONER);
	}
}
