 ...
 * 未初始化时从环境变量读取：</p>
 * <ul>
 *   <li>{@code SURE_AI_DEEPSEEK_API_KEY}（必填，缺失抛 {@link AiException}）</li>
 *   <li>{@code SURE_AI_DEEPSEEK_BASE_URL}（可选，覆盖默认 baseUrl）</li>
 * </ul>
 *
 * <p>DeepSeek 暂无 embeddings，本工具类仅提供对话与流式对话方法。</p>
 *
 * @author sureai
 * @since 0.1.0
 */
public final class DeepSeekUtil {

	/** 环境变量名：API Key。 */
	public static final String ENV_API_KEY = "SURE_AI_DEEPSEEK_API_KEY";
	...
	static AiConfig buildConfigFromEnv() {
		...
		if (apiKey == null || apiKey.isBlank()) {
			throw new AiException("未设置环境变量 " + ENV_API_KEY
				+ "，请先调用 DeepSeekUtil.init(apiKey) 或配置该环境变量");
		}
