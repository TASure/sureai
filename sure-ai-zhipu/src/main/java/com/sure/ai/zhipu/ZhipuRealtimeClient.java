...
/**
 * 智谱 GLM 实时语音（glm-realtime）客户端。
 *
 * <p>协议要点：</p>
 * <ul>
 *   <li>端点：{@code wss://open.bigmodel.cn/api/paas/v4/realtime?model=<model>}；</li>
 *   <li>鉴权：apiKey（{@code id.secret} 格式）先用 {@link ZhipuJwtGenerator} 签发 HS256 JWT，
 *       握手头 {@code Authorization: Bearer <jwt>}；</li>
 *   <li>下行按顶层 {@code type} 路由，与 OpenAI Realtime 协议兼容。</li>
 * </ul>
 *
 * @author sureai
 * @since 0.2.0
 */
public class ZhipuRealtimeClient extends AbstractRealtimeClient {

	/** 默认 WSS 基础地址。 */
	public static final String DEFAULT_WSS_BASE = "wss://open.bigmodel.cn/api/paas/v4";

	/** 实时模型。 */
	private final String model;

	/**
	 * 生产构造：用智谱 JWT 签发器生成 Bearer token 并通过默认连接器建连。
	 *
	 * @param config       连接配置（apiKey 为 id.secret 格式）
	 * @param model        实时模型（如 glm-realtime）
	 * @param eventListener 事件监听器
	 */
	public ZhipuRealtimeClient(AiConfig config, String model, RealtimeEventListener eventListener) {
		this(config, model, new DefaultRealtimeConnector(
			Map.of("Authorization", ZhipuJwtGenerator.generate(config.apiKey()))), eventListener);
	}
	 ...
	 * @param eventListener 事件监听器
	 */
	ZhipuRealtimeClient(AiConfig config, String model, RealtimeConnector connector,
