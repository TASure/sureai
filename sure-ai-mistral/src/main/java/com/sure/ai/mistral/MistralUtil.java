...
/**
 * Mistral AI 平台静态入口工具类。
 *
 * <p>双检锁懒加载单例：调用 {@link #init(String)} 或 {@link #init(AiConfig)} 显式初始化；
 * 未初始化时从环境变量读取：</p>
 * <ul>
 *   <li>{@code SURE_AI_MISTRAL_API_KEY}（必填，缺失抛 {@link AiException}）</li>
 *   <li>{@code SURE_AI_MISTRAL_BASE_URL}（可选，覆盖默认 baseUrl）</li>
 * </ul>
 *
 * @author sureai
 * @since 1.1.0
 */
public final class MistralUtil {

	/** 环境变量名：API Key。 */
	public static final String ENV_API_KEY = "SURE_AI_MISTRAL_API_KEY";
	...
	static AiConfig buildConfigFromEnv() {
		...
		if (apiKey == null || apiKey.isBlank()) {
			throw new AiException("未设置环境变量 " + ENV_API_KEY
				+ "，请先调用 MistralUtil.init(apiKey) 或配置该环境变量");
		}
	 ...
	 * 便捷向量。
	 *
	 * @param model 模型 ID（通常为 {@link MistralModels#MISTRAL_EMBED}）
	 * @param text  文本
	 * @return 响应
	 */
